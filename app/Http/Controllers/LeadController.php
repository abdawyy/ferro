...
use App\Jobs\SendAbandonedCartSequence;
...
class LeadController extends Controller
{
    ...
    private function quizProductsPayload($products, string $locale): array
    {
        return $products->map(function ($product) use ($locale) {
            return [
                ...
                'slug' => $product->slug,
                'image' => $product->featured_image ? ferro_public_url($product->featured_image) : null,
    ...
    public function trackAbandonedCart(Request $request): JsonResponse
    {
        ...
        dispatch(new SendAbandonedCartSequence($lead))
            ->delay(now()->addHour());
